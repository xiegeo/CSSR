 ...
 * changes:
 * pause and unpause
 * unified set and switch camera (and broken, create new preview to work around)
 */
...
class Preview extends ViewGroup implements SurfaceHolder.Callback {
    ...
    Preview(MainActivity a) {
        super(a);
        activity = a;
        setUp();
        Log.v(TAG, "new Preview");
    }
    ...
    public void setCamera(Camera camera) {
        ...
        if (mCamera != null) {
            ...
            requestLayout();
            mPreviewSize = getOptimalPreviewSize(mSupportedPreviewSizes, mtargetWidth, mtargetHeight);
            if (surfaceCreated) {
	...
	private void startPreview(int toTry)
	{
        Camera.Parameters parameters = mCamera.getParameters();
        parameters.setPreviewSize(mPreviewSize.width, mPreviewSize.height);
        mCamera.setParameters(parameters);
    	if (needBuffer) {
    		mCamera.addCallbackBuffer(new byte[CameraCallback.getBufferSize(mCamera)]);
    		needBuffer = false;
		}
        try{
        	...
        	if (toTry <= 1) {
        		...
        		Log.w(TAG,"try to start again:"+(toTry-1), e);
	        	activity.resetCamera();
	        	startPreview(toTry-1);
        	}
		}
	}
